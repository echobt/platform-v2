...
impl DistributedStorage {
    ...
    /// Receive a write proposal from another validator and validate before voting
    /// This is the P2P handler - other validators call this when they receive a proposal
    ///
    /// Returns: (voted_yes, consensus_result)
    /// - voted_yes: whether we voted YES or NO
    /// - consensus_result: Some(true) if committed, Some(false) if rejected, None if pending
    pub fn receive_write_proposal(
        &self,
        op: WriteOp,
        validate_fn: impl FnOnce(&WriteRequestInfo) -> WriteValidationResult,
    ) -> (bool, Option<bool>) {
        ...
        if self.pending_ops.read().contains_key(&op.op_id) {
            // Already received, just return current state
            return (false, None);
        }

        // Build validation info from the received entry
        let existing = self
            .get_raw(
                op.entry.header.category,
                &op.entry.header.challenge_id,
                &op.entry.header.key,
            )
            .ok()
            .flatten();
        ...
        let request_info = WriteRequestInfo {
            category: op.entry.header.category,
            challenge_id: op.entry.header.challenge_id.clone(),
            key: op.entry.header.key.clone(),
            value: op.entry.decompress_raw().unwrap_or_default(),
            size: op.entry.header.raw_size as usize,
            creator: op.entry.header.creator.clone(),
            creator_stake: 0, // We don't know their stake here
            block,
            is_update: existing.is_some(),
            previous_hash: existing.map(|e| e.header.value_hash),
            writes_this_epoch: 0, // Would need to track
            category_entry_count: self
                .count_category(&op.entry.header.challenge_id, op.entry.header.category),
            total_validators: total,
        };

        // Validate using challenge rules
        let approve = match validate_fn(&request_info) {
            WriteValidationResult::Accept => true,
            WriteValidationResult::Reject(reason) => {
                tracing::debug!("Rejecting write proposal: {}", reason);
                false
            }
        };

        // Store the op and add our vote
        let mut op = op;
        op.vote(&self.our_validator, approve);

        // Check if consensus already reached
        if let Some(result) = op.check_consensus(total) {
            if result {
                // Consensus reached - commit
                if let Err(e) = self.commit_write(op) {
                    tracing::error!("Failed to commit write: {}", e);
                    return (approve, Some(false));
                }
                self.stats.write().write_ops_committed += 1;
            } else {
                self.stats.write().write_ops_rejected += 1;
            }
            return (approve, Some(result));
        }

        // Store pending
        self.pending_ops.write().insert(op.op_id, op);
        self.stats.write().write_ops_pending = self.pending_ops.read().len();
